...
def main() -> None:
    ...
    ap.add_argument("--outdir", default="peer_minexp/history_contamination")
    ...
    detail_rows = []
    for delta in DELTAS:
        shock_log = float(np.log(1.0 - delta))
    ...
    primary = summary[summary["delta"] == 0.10][[
        "alpha", "contam_frac", "mean_recovery", "median_recovery",
        "mean_peer_overlap", "mean_benchmark_shift_log", "share_recovery_below_80pct"
    ]]
    lines.extend([
    ...
    print("=== PEER_HISTORY_FALSIFICATION ===")
    print("CLEAN_COMPARABILITY")
    print(comparability.round(5).to_string(index=False))
    print("PRIMARY_10PCT")
    print(primary.round(5).to_string(index=False))
    print("VERDICT")
    print(json.dumps(verdict, indent=2))
    print("=== END_PEER_HISTORY_FALSIFICATION ===")
